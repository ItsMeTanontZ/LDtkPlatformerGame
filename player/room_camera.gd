...
func _ready() -> void:
	# Initialize camera position to the room the player starts in
	var player_pos = get_parent().global_position
	
	# Calculate which room the player is in
	current_room_x = floor(player_pos.x / ROOM_WIDTH)
	current_room_y = floor(player_pos.y / ROOM_HEIGHT)
	
	# Position camera at the center of the current room immediately
